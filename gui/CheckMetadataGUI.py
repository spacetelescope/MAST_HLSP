...
from CHECK_METADATA_FORMAT.get_filetypes_keys import get_filetypes_keys
...
BASE = "/".join(CURRENT_DIR.split("/")[:-1])
TEMPLATE_TYPES_FILE = os.path.join(
    BASE, "CHECK_METADATA_FORMAT/allowed_template_types.dat")
FILE_TYPES_FILE = os.path.join(
    BASE, "CHECK_METADATA_FORMAT/allowed_file_types.dat")
PRODUCT_TYPES_FILE = os.path.join(
    BASE, "CHECK_METADATA_FORMAT/allowed_product_types.dat")
CAOMPRODUCT_TYPES_FILE = os.path.join(
    BASE, "CHECK_METADATA_FORMAT/allowed_caomproduct_types.dat")
...
def read_allowed_template_types():
    ...
    if os.path.isfile(TEMPLATE_TYPES_FILE):
        ...
        raise OSError('Allowed Template Types file not found.  Looking for "' +
                      os.path.abspath(TEMPLATE_TYPES_FILE) + '".')
...
class CheckMetadataGUI(QtWidgets.QWidget):
    """
    ...
    def set_default_product_type(self, widget, ending):
        ...
        index = self.get_yaml_index(ending)
        cur_value = self.param_data['FileTypes'][index[0]
                                                 ][ending]['ProductType']
        if cur_value is not None:
            widget.setCurrentIndex(widget.findText(cur_value))
        else:
    ...
    def init_ui(self):
        ...
        #input_dir_label = QtWidgets.QLabel("<placeholder>")
        ...
        k = "a b c d e f g".split(" ")
        v = list(range(len(k)))
        test = dict(zip(k, v))
        # for extension in test
        for ending in test.keys():
            ...
            # Define a default or load previous.
            #self.set_default_caomproduct_type(this_caomproduct_widget, ending)

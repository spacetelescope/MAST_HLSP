...
class SelectDataTemplatesGUI(QtWidgets.QWidget):
    ...
    def get_yaml_index(self, exten, ending):
        """
        Gets the index to use in the YAML data object for the given exten
            and ending.
    ...
    def update_yaml_value(self, exten, ending, update_key, update_value):
        ...
        index = self.get_yaml_index(exten, ending)
        self.param_data[exten][index[0]]['FileParams'][update_key] = (
            update_value)

    def set_default_standard(self, widget, exten, ending):
        """
        ...
        index = self.get_yaml_index(exten, ending)
        cur_value = self.param_data[exten][index[0]]['FileParams']['Standard']
        if cur_value is not None:
            ...
            self.update_yaml_value(exten, ending, 'Standard', None)

    def set_default_product_type(self, widget, exten, ending):
        """
        ...
        index = self.get_yaml_index(exten, ending)
        cur_value = self.param_data[exten][index[0]]['FileParams']['ProductType']
        if cur_value is not None:
            widget.setCurrentIndex(widget.findText(cur_value))
        else:
            # Default for first run through is everything set to none.
            widget.setCurrentIndex(widget.findText('none'))
            self.update_yaml_value(exten, ending, 'ProductType', None)
    ...
    def set_default_file_type(self, widget, exten, ending):
        ...
        index = self.get_yaml_index(exten, ending)
        cur_value = self.param_data[exten][index[0]]['FileParams']['FileType']
        if cur_value is not None:
            widget.setCurrentIndex(widget.findText(cur_value))
        else:
            if exten == 'fits':
    ...
    def set_default_runcheck(self, widget, exten, ending):
        """
        ...
        index = self.get_yaml_index(exten, ending)
        cur_value = self.param_data[exten][index[0]]['FileParams']['RunCheck']
        if cur_value is not None:
    ...
    def set_default_mrpcheck(self, widget, exten, ending):
        """
        ...
        index = self.get_yaml_index(exten, ending)
        cur_value = self.param_data[exten][index[0]]['FileParams']['MRPCheck']
        if cur_value is not None:
            widget.setChecked(cur_value)
        else:
            # Default for first run through is everything set to MRP = True.
            widget.setChecked(True)
            self.update_yaml_value(exten, ending, 'MRPCheck', True)

    def set_default_caomproduct_type(self, widget, exten, ending):
        """
        ...
        index = self.get_yaml_index(exten, ending)
        cur_value = (
            self.param_data[exten][index[0]]['FileParams']['CAOMProductType'])
        if cur_value is not None:
            widget.setCurrentIndex(widget.findText(cur_value))
        else:
            # Default for first run through is everything set to science.
            widget.setCurrentIndex(widget.findText('science'))
            self.update_yaml_value(exten, ending, 'CAOMProductType', None)
        ...
        for extension in self.param_data.keys():
            if extension != 'InputDir':
                ...
                for ending in self.param_data[extension]:
                    ...
                    self.set_default_standard(this_template_widget, extension,
                                              ending)
                    ...
                    # Define a default or load previous.
                    self.set_default_product_type(this_product_widget,
                                                  extension, ending)
                    ...
                    self.set_default_runcheck(this_run_checkbox, extension,
                                              ending)
                    ...
                    self.set_default_mrpcheck(this_mrp_checkbox, extension,
                                              ending)
